...
def evaluate_models(
    ...
    fold_splits = [(train_idx, val_idx) for train_idx, val_idx, _ in fold_indices]

    xgb_params = tune_xgb_position(X, y_position, fold_splits, optuna_trials)
    lgb_params = tune_lgb_time(X, y_time, fold_splits, optuna_trials)
